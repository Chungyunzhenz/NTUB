...
app.config['MYSQL_HOST'] = '0.0.0.242'
app.config['MYSQL_USER'] = '0'
app.config['MYSQL_PASSWORD'] = '0$0'
app.config['MYSQL_DB'] = '113-0'
...
def upload_image():
    ...
    connection.commit()
    cursor.execute("SELECT ID FROM ImageUploads WHERE Image = %s AND  UploadDate = %s AND UploadedBy = %s ORDER BY ID DESC LIMIT 1",
                   (image_data,upload_date, uploaded_by))
