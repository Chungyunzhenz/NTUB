...
try:
    ...
    current_app.logger.error(f"Failed to load CKIPtagger models: {e}")
    raise
...
def connect_neo4j():
    try:
        ...
        current_app.logger.error(f"Neo4j connection failed: {e}")
        return None
...
def load_custom_dict(file_path):
    ...
    try:
        ...
        current_app.logger.error(f"Failed to load custom dictionary: {e}")
    return custom_dict
...
def check_and_load_custom_dict(file_path):
    ...
    try:
        ...
        if custom_dict_last_modified is None or last_modified > custom_dict_last_modified:
            current_app.logger.info("Reloading custom dictionary due to file modification.")
            custom_dict_cache = load_custom_dict(file_path)
            custom_dict_last_modified = last_modified
    except Exception as e:
        current_app.logger.error(f"Failed to check custom dictionary file: {e}")

    return custom_dict_cache

@z5.route('/predict', methods=['POST'])
def predict():
    # 從請求中獲取句子
    data = request.json
    ...
    try:
        ...
        # 記錄 WS 和 POS 結果以便調試
        current_app.logger.info(f"WS results: {ws_results}")
        current_app.logger.info(f"POS results: {pos_results}")
        
        # 執行 NER
        ner_results = ner(ws_results, pos_results)
        current_app.logger.info(f"NER results: {ner_results}")
    except Exception as e:
        current_app.logger.error(f"NER prediction failed: {e}")
    ...
    try:
        ...
        if (not ner_results or not isinstance(ner_results, list) or not ner_results[0]) and \
            ...
            current_app.logger.error("No valid NER or POS results.")
        ...
        try:
            ...
            current_app.logger.error(f"Failed to insert data into the database: {e}")
        ...
        current_app.logger.error(f"An error occurred during processing: {e}")
