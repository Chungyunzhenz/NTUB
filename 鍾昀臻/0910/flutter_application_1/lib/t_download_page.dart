...
class FormDownloadPageState extends State<FormDownloadPage> {
  ...
  final Map<String, List<String>> _studentSemesterData = {
    '張偉': ['學期一請假單.pdf', '學期二請假單.pdf'],
    '李強': ['學期一請假單.pdf', '學期二請假單.pdf'],
    // 其他學生...
  };

  List<Map<String, dynamic>> get _filteredStudents {
    if (_selectedClass == null) return [];
    var students = _classData[_selectedClass]!;
    if (_searchKeyword.isNotEmpty) {
      students = students
          .where(
              (student) => student['name'].toString().contains(_searchKeyword))
          .toList();
    }
    return students;
  }

  List<String> get _filteredSemesters {
    if (_selectedStudent == null) return [];
    var semesters = _studentSemesterData[_selectedStudent]!;
    if (_searchKeyword.isNotEmpty) {
      semesters = semesters
          .where((semester) => semester.contains(_searchKeyword))
          .toList();
    }
    return semesters;
  }
                ...
                : _selectedStudent == null
                    ...
                    : Expanded(
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            String semester = _filteredSemesters[index];
                            return ListTile(
                              leading: Icon(
                                Icons.file_present,
                                color: Colors.green[400],
                              ...
                              title: Text(semester),
                              onTap: () {
                                _showDetailsDialog(context, semester);
                              },
                            );
                          },
                        ),
