  ...
  Future<void> _fetchCourseRequests() async {
    ...
    try {
      ...
      final pendingResponse = await http.get(
        Uri.parse(
            'http://$serverIp:3000/getLeaveRequests?status=審查中&title=選課單'),
      );
      final returnedResponse = await http.get(
        Uri.parse('http://$serverIp:3000/getLeaveRequests?status=退回&title=選課單'),
      );
      final completedResponse = await http.get(
        Uri.parse('http://$serverIp:3000/getLeaveRequests?status=通過&title=選課單'),
      );
